...
class PriorityQueue:
    ...
    def __init__(self, order='min', f=lambda x: x):
        ...
        if order == 'min':
            ...
            self.f = lambda x: -f(
                x)  # now item with max f(x) will be popped first.
        else:
...
def sequence(iterable):
    ...
    return (iterable if isinstance(iterable, collections.abc.Sequence) else
            tuple([iterable]))
...
def power_set(iterable):
    ...
    s = list(iterable)
    return list(
        chain.from_iterable(combinations(s, r) for r in range(len(s) + 1)))[1:]
...
def scalar_vector_product(x, y):
    ...
    return [scalar_vector_product(x, _y)
            for _y in y] if hasattr(y, '__iter__') else x * y


def map_vector(f, x):
    """Apply function f to iterable x."""
    return [map_vector(f, _x)
            for _x in x] if hasattr(x, '__iter__') else list(map(f, [x]))[0]
...
def cross_entropy_loss(x, y):
    ...
    return (-1.0 / len(x)) * sum(x * np.log(_y) + (1 - _x) * np.log(1 - _y)
...
def gaussian(mean, st_dev, x):
    ...
    return 1 / (np.sqrt(2 * np.pi) * st_dev) * np.exp(
        -0.5 * (float(x - mean) / st_dev)**2)
...
def rbf_kernel(x, y=None, gamma=None):
    ...
    return np.exp(-gamma *
                  (-2.0 * np.dot(x, y.T) + np.sum(x * x, axis=1).reshape(
                      (-1, 1)) + np.sum(y * y, axis=1).reshape((1, -1))))
...
def ucb(n, C=1.4):
    return np.inf if n.N == 0 else n.U / n.N + C * np.sqrt(
        np.log(n.parent.N) / n.N)
